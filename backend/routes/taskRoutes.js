...
const {pushTaskSync, pushTaskDelete} = require('../services/uapSync');
...
router.post("/", requireAuth, async (req, res) => {
    try {
        ...
        // Sync the new task to UAP. Failure here does not block the response -
        // pushTaskSync handles its own retries and logs failures internally.
        pushTaskSync(populatedTask).catch(err =>
            console.error('UAP sync (create) failed unexpectedly:', err)
        );
...
router.delete("/:id", requireAuth, requireRole('Supervisor', 'Admin'), async (req, res) => {
    try {
        ...
        // Notify UAP the task is gone so their copy is removed too.
        pushTaskDelete(req.params.id).catch(err =>
            console.error('UAP sync (delete) failed unexpectedly:', err)
        );
...
router.patch("/:id/status", requireAuth, async (req, res) => {
    try {
        ...
        pushTaskSync(updatedTask).catch(err =>
            console.error('UAP sync (status change) failed unexpectedly:', err)
        );
...
router.patch("/:id", requireAuth, async (req, res) => {
    try {
        ...
        pushTaskSync(updatedTask).catch(err =>
            console.error('UAP sync (update) failed unexpectedly:', err)
        );
