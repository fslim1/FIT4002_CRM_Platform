...
import { FiAlertCircle, FiCheckCircle, FiPlus, FiSearch, FiTrash2, FiUsers, FiX, FiEye, FiEyeOff, FiShield } from 'react-icons/fi';
import { fetchAdminUsers, createUser, deleteUser } from '../api/users';
...
export default function AdminUsers() {
  ...
  const validateForm = () => {
    ...
    if (!CREATABLE_ROLES.includes(form.role)) return 'Please select a valid role.';
    return null;
  };
  ...
  const pwStrength = (() => {
    ...
    if (score <= 1) return { label: 'Weak', cls: 'pw-weak' };
    if (score === 2) return { label: 'Fair', cls: 'pw-fair' };
    if (score === 3) return { label: 'Good', cls: 'pw-good' };
      ...
      <div className="admin-container">
        <div className="admin-header">
          ...
          <p className="admin-subtitle">
            Create and manage salesperson and supervisor accounts for your company.
          </p>
        </div>
                  ...
                  {users.map((u) => {
                    ...
                    return (
                      <tr key={u.id} className={!u.isActive ? 'aum-row-inactive' : ''}>
                        ...
                        <td>
                          {u.team?.name
                            ? <span className="aum-team-name">{u.team.name}</span>
                            : <em className="aum-no-team">No team</em>}
                        </td>
                        <td>
          ...
          <div className="aum-modal">
            <div className="aum-modal-header">
              <div>
                ...
                <p>Add a new salesperson or supervisor to your company.</p>
              </div>
              <button className="aum-modal-close" onClick={closeCreate} disabled={creating} aria-label="Close">
                <FiX />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSubmit} noValidate className="aum-modal-form">
              ...
              <div className="aum-field">
                <label htmlFor="aum-password">Temporary Password <span className="aum-req">*</span></label>
                <div className="aum-pass-wrap">
                  <input
                    ...
                    disabled={creating}
                  />
                  <button type="button" className="aum-pass-toggle" onClick={() => setShowPass((v) => !v)} tabIndex={-1} aria-label={showPass ? 'Hide password' : 'Show password'}>
                    {showPass ? <FiEyeOff /> : <FiEye />}
                  </button>
                </div>
                {pwStrength && (
                  <div className="aum-pw-bar">
                    <div className={'aum-pw-fill ' + pwStrength.cls} />
                    <span className={'aum-pw-label ' + pwStrength.cls}>{pwStrength.label}</span>
                  </div>
                )}
              </div>
              ...
              <div className="aum-field">
                ...
                <div className="aum-pass-wrap">
                  <input
                    ...
                    disabled={creating}
                  />
                  <button type="button" className="aum-pass-toggle" onClick={() => setShowConfirmPass((v) => !v)} tabIndex={-1} aria-label={showConfirmPass ? 'Hide' : 'Show'}>
              ...
              <div className="aum-field">
                ...
                <span className="aum-field-hint">
                  <FiShield style={{ fontSize: 11 }} /> Admin accounts can only be created via the server seed script.
                </span>
              </div>

              {teams.length > 0 && (
                <div className="aum-field">
                  <label htmlFor="aum-teamId">Team <span className="aum-optional">(optional)</span></label>
                  <select
                    id="aum-teamId"
                    name="teamId"
                    className="admin-select"
              ...
              <div className="aum-modal-actions">
                <button type="button" className="aum-btn-secondary" onClick={closeCreate} disabled={creating}>
                  Cancel
                </button>
                <button id="aum-submit-create" type="submit" className="aum-btn-primary" disabled={creating}>
        ...
        <div className="aum-overlay" role="dialog" aria-modal="true" aria-labelledby="aum-del-title">
          <div className="aum-modal aum-modal-sm">
            ...
            <p className="aum-del-desc">
              You are about to permanently delete <strong>{deleteTarget.fullName}</strong>&apos;s account ({deleteTarget.email}).
              <br /><br />
              Their deals, tasks and customers will be preserved but ownership attribution will be cleared.
              This action <strong>cannot be undone</strong>. Consider deactivating instead if you want to block access while keeping history intact.
            </p>
            <div className="aum-modal-actions">
              <button
                id="aum-del-cancel"
